...
static bool json_extract_string(const char *json, const char *key, char *buffer, size_t capacity) {
    ...
    while (*p && write_pos + 1 < capacity) {
        if (*p == '\\') {
            // Escape sequence: support common escapes \" \\ \/ \b \f \n \r \t and \uXXXX minimally
            ++p;
            if (!*p) break;
            switch (*p) {
                ...
                case 'u': {
                    // Minimal handling: skip \uXXXX sequence and emit '?' placeholder if capacity allows
                    // This parser avoids complex unicode decoding; callers should validate if unicode is required.
                    ++p;
                    int hex_count = 0;
                ...
                default:
                    // Unknown escape: copy character as-is
                    buffer[write_pos++] = *p;
                    ++p;
                    break;
            }
    ...
    // If we exit loop without finding closing quote, fail
    return false;
}
...
static bool parse_rational_string(const char *text, mpq_t value) {
    ...
    // Use GMP's mpz_set_str for arbitrary precision
    mpz_t numz, denz;
    mpz_init(numz);
    mpz_init(denz);
...
bool config_load_from_file(Config *config, const char *path, char *error_buffer,
    ...
    apply_optional_enum(json, "ratio_trigger_mode", RATIO_TRIGGER_NONE, RATIO_TRIGGER_CUSTOM,
                        &enum_value);
    ...
    // Load custom ratio bounds if present
